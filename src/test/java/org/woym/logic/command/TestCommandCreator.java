...
public class TestCommandCreator {
	...
	public void testRoom() throws Exception {
		...
		assertEquals(7, macro.getCommands().size());
	...
	public void testLocation() throws Exception {
		...
		assertEquals(9, macro.getCommands().size());

		assertTrue(macro.getCommands().get(8) instanceof DeleteCommand);
	...
	public void testActivityTypeLessondType() throws Exception {
		...
		IMemento mementoSchoolClass = Mockito.mock(org.woym.objects.Schoolclass.Memento.class);
		IMemento mementoYear = Mockito.mock(org.woym.objects.AcademicYear.Memento.class);
		...
		List<CompoundLesson> compoundLessons = Arrays.asList(compoundLesson1,
				compoundLesson2);
		List<Schoolclass> schoolClasses = Arrays.asList(schoolclass,schoolclass);
		...
		Mockito.when(DataAccess.getInstance()
				.getAllSchoolclasses()).thenReturn(schoolClasses);
		Mockito.when(schoolclass.createMemento()).thenReturn((org.woym.objects.Schoolclass.Memento) mementoSchoolClass);
		
		Mockito.when( DataAccess.getInstance()
					.getAllAcademicYears()).thenReturn(years);
		Mockito.when(year.createMemento()).thenReturn((org.woym.objects.AcademicYear.Memento) mementoYear);
		...
		assertEquals(13, macro.getCommands().size());
